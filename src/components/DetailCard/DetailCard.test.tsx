...
import DetailCard, { detailCardLoader } from './DetailCard';
...
describe('Detail Card', () => {
  ...
  spy.mockResolvedValue({ detailedCard: data[0] });
  ...
  it('Make sure the detailed card component correctly displays the detailed card data and close after press button', async () => {
    ...
    expect(screen.queryByTestId('24 min per ep')).not.toBeInTheDocument();
    expect(screen.queryByTestId('R - 17+ (violence & profanity)')).not.toBeInTheDocument();
    expect(screen.queryByTestId('TV 26 episodes')).not.toBeInTheDocument();
    expect(screen.queryByTestId('Finished Airing')).not.toBeInTheDocument();
  });
});
